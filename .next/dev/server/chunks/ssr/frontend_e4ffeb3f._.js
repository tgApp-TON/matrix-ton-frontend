...
function TablesContent() {
    ...
    const fetchUserByWallet = async (wallet)=>{
        try {
            const response = await fetch(`/api/user/by-wallet?wallet=${wallet}`);
            const data = await response.json();
            if (data.success && data.user) {
                // User exists - load their tables
                setUserId(data.user.id.toString());
                localStorage.setItem('matrix_ton_user_id', data.user.id.toString());
                fetchUserTables(data.user.id.toString());
            } else {
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$frontend$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useEffect"])(()=>{
        if (address) {
            ...
            const id = localStorage.getItem('matrix_ton_user_id') || '1';
            setUserId(id);
            fetchUserTables(id);
            setLoading(false);
        }
...
//# sourceMappingURL=frontend_e4ffeb3f._.js.map