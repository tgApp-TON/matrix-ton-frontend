...
"[project]/frontend/app/api/user/tables/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    ...
    try {
        // Get user's tables
        const { data: tables, error } = await __TURBOPACK__imported__module__$5b$project$5d2f$frontend$2f$lib$2f$supabase$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["supabase"].from('Table').select(`
        id,
        tableNumber,
        status,
        cycleNumber,
        createdAt,
        closedAt
      `).eq('userId', parseInt(userId)).order('tableNumber', {
            ascending: true
        });
        if (error) throw error;
        // Get positions for each table
        const tablesWithPositions = await Promise.all((tables || []).map(async (table)=>{
            const { data: positions } = await __TURBOPACK__imported__module__$5b$project$5d2f$frontend$2f$lib$2f$supabase$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["supabase"].from('TablePosition').select('id, position, partnerUserId, amountPaid, status').eq('tableId', table.id).order('position', {
                ascending: true
            });
            return {
                ...table,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__572963a2._.js.map